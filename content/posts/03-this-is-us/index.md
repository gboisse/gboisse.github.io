...
A quick word on specular; it uses about the same data flow as diffuse but with a few additional dedicated heuristics.
...
I ended up using the exact same data structure as the one used for the SPH simulation, but this time I subdivided each cell further into either a 2x2x2 or 4x4x4 subdivision (left as a tweakable option of the algorithm).
...
We then end up with a complete and unified shadowing system. :slightly_smiling_face:
